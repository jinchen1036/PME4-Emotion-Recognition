function [emg_mfcc_data] = emg_mfcc_avg(data, datapath, dataname, Tw,Ts)
    addpath('') % datapath
    ...
    emg_bootstrap_avg_mfcc_train_data = readNPY(sprintf('%s/%s_5kHz_avg_mfcc_%dms_%dms_train.npy', datapath,dataname, Tw, Ts));
    emg_bootstrap_avg_mfcc_test_data = readNPY(sprintf('%s/%s_5kHz_avg_mfcc_%dms_%dms_test.npy', datapath,dataname, Tw, Ts));
        ...
        for emo = 1:numel(emo_fn)
            ...
            for trial = 1 : size(train_group_index,1)
                ...
                for channel = 1 : size(group_data,2)
                    emg_bootstrap_avg_data = reshape(mean(group_data(:,channel,:),1),1,[]);
                    [norm_MFCCs, norm_FBEs, norm_frames ] = mfcc(emg_bootstrap_avg_data,fs, Tw, Ts, alpha, @hamming, [LF HF], M, C, L );
                    emg_bootstrap_avg_mfcc_train_data(sub,emo,trial,channel,:,:) = norm_MFCCs.';
                end
            end
           ...
           for trial = 1 : size(test_group_index,1)
                ...
                for channel = 1 : size(group_data,2)
                    emg_bootstrap_avg_data = reshape(mean(group_data(:,channel,:),1),1,[]);
                    [norm_MFCCs, norm_FBEs, norm_frames ] = mfcc(emg_bootstrap_avg_data,fs, Tw, Ts, alpha, @hamming, [LF HF], M, C, L );
                    emg_bootstrap_avg_mfcc_test_data(sub,emo,trial,channel,:,:) = norm_MFCCs.';
                end
           end
        end

        filename = sprintf('%s/%s_5kHz_avg_mfcc_%dms_%dms_train.npy', datapath,dataname, Tw, Ts);
        writeNPY(emg_bootstrap_avg_mfcc_train_data, filename);

        filename = sprintf('%s/%s_5kHz_avg_mfcc_%dms_%dms_test.npy', datapath,dataname, Tw, Ts);
        writeNPY(emg_bootstrap_avg_mfcc_test_data, filename);
